...
class RelationModel extends Model {
    ...
    protected function _after_insert($data,$options) {
        ...
        // $result['success']=false;$result['msg']=json_encode($data);exit(json_encode($result));
        if(!empty($options['link']))
    ...
    protected function _after_update($data,$options) {
        // 关联更新
        // $result['success']=false;$result['msg']='M='.json_encode($options);exit(json_encode($result));
        $data   = array_merge($this->_data,$data);
        // $result['success']=false;$result['msg']=json_encode($data);exit(json_encode($result));
        if(!empty($options['link']))
            ...
            foreach($this->_link as $key=>$val) {
                    ...
                    if(empty($name) || true === $name || $mappingName == $name || (is_array($name) && in_array($mappingName,$name))) {
                        ...
                        if(!empty($val['condition'])){
                            $mappingCondition  = preg_replace_callback('/{(\w+)}/',function($match){return PUT($match[1]);},$mappingCondition);
                        }
        ...
        if(!empty($this->_link)) {
            ...
            foreach($this->_link as $key=>$val) {
                    ...
                    if(empty($name) || true === $name || $mappingName == $name || (is_array($name) && in_array($mappingName,$name)) ) {
                        ...
                        if(!empty($mappingData) || $opType == 'DEL' || self::HAS_MANY && $opType == 'SAVE') {
                            switch($mappingType) {
                                case self::HAS_ONE:
                                    switch (strtoupper($opType)){
                                        ...
                                        $this->autoData($mappingData,self::MODEL_INSERT,$fields);
                                        ...
                                        $map        = array();
                                        $map[$mappingFk]    =   $pk;
                                        $node   = $model->where($map)->find();
                                        // dump($mappingData);exit;
                                        // print_r($map);
                                        // $pk1   =  $model->getPk();
                                        // $result['success']=false;$result['msg']='mfk';exit(json_encode($result));
                                        if(!is_null($node)) {
                                            $this->autoData($mappingData,self::MODEL_INSERT,$fields);
                                            // dump($mappingData);
                                            $result   =  $model->where($mappingCondition)->save($mappingData);
                                        }else{
                                        // $result['success']=false;$result['msg']=json_encode($pk);exit(json_encode($result));
                                            $mappingData[$mappingFk]    =   $pk;
                                            // $this->error($model->create($mappingData));
                                            $this->autoData($mappingData,self::MODEL_INSERT,$fields);
                                            $result   =  $model->add($mappingData);
                                            // $this->error($result);
                                        }
                                ...
                                case self::HAS_MANY:
                                    switch (strtoupper($opType)){
                                        case 'ADD'   :  // 增加关联数据
                                        $model->startTrans();
                                        // $result['success']=false;$result['msg']=json_encode($mappingFk);exit(json_encode($result));
                                        $fields     = $model->getDbFields();
                                        foreach ($mappingData as $val){
                                            $this->autoData($val,self::MODEL_INSERT,$fields);
                                            $val[$mappingFk]    =   $pk;
                                        ...
                                        $model->startTrans();
                                        $pk   =  $model->getPk();
                                        // $result['success']=false;$result['msg']=json_encode($mappingData);exit(json_encode($result));
                                        foreach($mappingData as $v){
                                            if($v[$pk])
                                                $ret[]	= $v[$pk];
                                        }
                                        $map				= array();
                                        if(count($ret)>0){
    										$map[$pk]		= array('not in',$ret);
    										$model->where($map);
                                        }
                                        $fields     = $model->getDbFields();
                                        $model->where($mappingCondition)->delete();
                                        foreach ($mappingData as $vo){
                                            if(isset($vo[$pk])&&$vo[$pk]) {// 更新数据
                                                $mappingCondition   =  "$pk ={$vo[$pk]}";
                                                $this->autoData($vo,self::MODEL_UPDATE,$fields);
                                                // $result['success']=false;$result['msg']=json_encode($fields);exit(json_encode($result));
                                                $result   =  $model->where($mappingCondition)->save($vo);
                                            }else{ // 新增数据
                                                $vo[$mappingFk] =  $data[$mappingKey];
                                                // $result['success']=false;$result['msg']=json_encode($fields);exit(json_encode($result));
                                                $this->autoData($vo,self::MODEL_INSERT,$fields);
                                                $result   =  $model->add($vo);
                                            }
                                        }
                                        $model->commit();
                                ...
                                case self::MANY_TO_MANY:
                                    ...
                                    // $result['success']=false;$result['msg']=json_encode($mappingKey);exit(json_encode($result));
                                    switch (strtoupper($opType)){
                                        case 'ADD': // 增加关联数据
                                            if(isset($relationId)) {
                                                $this->startTrans();
     ...
     * @return mixed
     */
     private function autoData(&$data='',$type='',$fields) {
